...
/**
 * 有一些积木（多为头积木）在折叠时需要隐藏子积木
 * 在这些积木中，next_statement不会被设置为true
 * 子积木的connection不直接连接在积木上（next_connection总是为undefined）
 * 而是连接在inputList最后名为DO或STACK的input.connection上
 * 很多地方需要对这一类积木进行特殊处理
 */
exports.SPECIAL_CONNECTION_INPUT_NAME = [
    'DO',
    'STACK',
];
/**
 * use for the block config { nextStatement: true } but need to collapse children blocks
 */
exports.NEED_COLLAPSE_CHILDREN_BLOCKS = [
    // TODO: 下面这块积木 kitten 计划在 heart 里面重新配置，这里也许不需要了
    // 反正到今天也没有，留着吧 = =
    'start_on_click',
    // 以下 nemo 积木使用了错误的配置积木 { nextStatement: true }
    // 在 theatre-mobile 重新配置正确就可以不写在这里了
    'self_on_tap',
