...
var h = function () {
    ...
    try {
      for (var s = 0; s < r; s++) {
        ...
        if ("block" == l || "shadow" == l && !this.events.is_record_undo() || "empty" == l && !this.events.is_record_undo() ? f = this.dom_to_block(u, t) : "shadow" === l || "empty" === l ? console.error("Shadow block cannot be a top-level block.") : "comment" === l && (f = this.dom_to_workspace_comment(u, t)), f) {
          n.push(f.id);
          var h = parseInt(u.getAttribute("x") || "0", 10);
          var d = parseInt(u.getAttribute("y") || "0", 10);
          if (!(isNaN(h) || isNaN(d))) {
            h = t.RTL ? 0 - h : h;
            if ((0, c.is_block_svg)(f) && f.parent_group) {
              f.parent_group.move_by(o.vec2.fromValues(h, d));
            } else {
              f.move_by(o.vec2.fromValues(h, d));
            }
          }
        }
      }
  ...
  e.prototype.block_to_dom = function (e, t) {
    ...
    if ((0, c.is_block_group)(e)) {
      ...
      var h = l && this.block_to_dom(l, t);
      if (h) {
        h.setAttribute(a.BLOCK_GROUP_STOPPED_XML_TAG, "true");
        var d = i.querySelector("next[" + a.LAST_NEXT_XML_TAG + '="true"]');
        if (!(null === d || void 0 === d)) {
          d.removeAttribute(a.LAST_NEXT_XML_TAG);
        }
        if (!(null === d || void 0 === d)) {
          d.appendChild(h);
        }
  ...
  e.prototype.dom_to_block_headless = function (e, t, n) {
    ...
    if ("true" === e.getAttribute(a.BLOCK_GROUP_STOPPED_XML_TAG)) {
      for (var f = this.grouped_blocks.length - 1; f >= 0; f--) {
        var h = this.grouped_blocks[f];
        if (!(h.block.is_starting_block() || h.block.is_output_block() || void 0 !== h.group_stop_at)) {
          h.group_stop_at = c || s.id;
        }
      }
    }
    if (c && u) {
    ...
    for (f = 0; f < e.childNodes.length; f++) {
      ...
      if (d.nodeType != a.NodeType.TEXT) {
        for (var p = void 0, _ = void 0, A = 0; A < d.childNodes.length; A++) {
          var g = d.childNodes[A];
        ...
        switch (d.nodeName.toLowerCase()) {
          case "mutation":
            ...
            break;
          case "comment":
            s.set_comment_text("", d.getAttribute("id") || void 0);
            this.parse_comment_attributes(d, s.comment);
            break;
          case "data":
            s.data = d.textContent;
            break;
          case "title":
          case "field":
            this.parse_child_field(d, s);
            break;
          case "value":
          case "statement":
            var m = d.getAttribute("name"),
              y = s.get_input(m);
            if (!y || !y.connection) {
    ...
    if (O) {
      var S = void 0;
      S = "false" === O || O === a.BlockVisibility.TRANSLUCENT ? a.BlockVisibility.TRANSLUCENT : "hidden" === O ? a.BlockVisibility.HIDDEN : a.BlockVisibility.VISIBLE;
      s.set_chunk_visibility(S);
    }
    if ("shadow" === o || "empty" === o) {
      var k = s.get_children();
    ...
    if (void 0 != n) {
      ...
      if (r) {
        ...
        if ((0, c.is_field_number)(r)) {
          ...
          var d = e.getAttribute("controller");
          if (d) {
            var p = void 0;
            p = "SLIDER" === (l = d.split(","))[0] ? {
