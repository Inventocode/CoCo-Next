...
exports.load_from_xml = function (e, t, n) {
  ...
  if (s.forEach(function (t) {
    ...
    if (o === i.PROCEDURE_BLOCK_TYPES.DEF) {
      ...
      var h = f && f.querySelectorAll('block[type="' + i.PROCEDURE_BLOCK_TYPES.RETURN + '"]>mutation[items="1"]').length || 0;
      if (!n) {
        var d = e.add_procedure_def(s, a);
        d.params = l;
        d.return_count = h;
        return void (p && d.name !== s && (p.textContent = d.name));
      }
      if (!(_ = c[s])) {
        _ = {
          def_id: a,
