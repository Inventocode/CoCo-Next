...
exports.position_dropdown = function (e, t) {
  ...
  var h = t.source_block.workspace.options.dropdown;
  var d = h.menu.scalable ? Math.max(u, 1) : 1;
  if (e.parentElement) {
    e.parentElement.style.transform = "scale(" + d + ")";
  ...
  if (c[1] <= a.height / 2 + s[1] ? (c[1] += l.height + g, e.classList.add("lower"), _.classList.add("lower")) : (c[1] -= p.height + g, e.classList.add("upper"), _.classList.add("upper"), _.style.top = p.height + "px"), (0, i.insert_before)(_, e), "first" === h.menu.align) {
    var v = t.image_element ? Number(t.image_element.getAttribute("x")) : t.text_element ? Number(t.text_element.getAttribute("x")) : NaN;
    v = isNaN(v) ? h.padding : v;
  ...
  } else if ("center" === h.menu.align) {
    if (e.parentElement) {
      e.parentElement.style.transform = "scale(" + d + ") translateX(-50%)";
    }
    c[0] += l.width / 2;
    c[0] = Math.min(c[0], a.width + s[0] - p.width / 2);
    c[0] = Math.max(c[0], s[0] + p.width / 2);
  } else {
    if ("left" === h.menu.align) {
      c[0] = Math.min(c[0], a.width + s[0] - p.width);
    }
  }
  return c;
