...
var o = require(/* 151 */"../151")
var a = require(/* 170 */"../170")
var s = require(/* 73 */"../73/index")
...
var f = function (t) {
  ...
  e.checkStandardUPCEANChecksum = function (t) {
    ...
    var r = parseInt(t.charAt(t$length - 1), 10)
    return e.getStandardUPCEANChecksum(t.substring(0, t$length - 1)) === r
  }
  e.getStandardUPCEANChecksum = function (t) {
    for (var /* [auto-meaningful-name] */t$length = t.length, n = 0, r = t$length - 1; r >= 0; r -= 2) {
  ...
  e.findGuardPattern = function (t, n, r, i, o) {
    for (var a = t.getSize(), s = 0, l = n = r ? t.getNextUnset(n) : t.getNextSet(n), /* [auto-meaningful-name] */i$length = i.length, f = r, h = n; h < a; h++) {
      if (t.get(h) !== f) {
        o[s]++
      } else {
        if (s === i$length - 1) {
  ...
  e.decodeDigit = function (t, n, r, i) {
    ...
    for (var /* [auto-meaningful-name] */this$MAX_AVG_VARIANCE = this.MAX_AVG_VARIANCE, a = -1, /* [auto-meaningful-name] */i$length = i.length, l = 0; l < i$length; l++) {
      var d = i[l]
      var f = u.a.patternMatchVariance(n, d, e.MAX_INDIVIDUAL_VARIANCE)
      if (f < this$MAX_AVG_VARIANCE) {
        this$MAX_AVG_VARIANCE = f
        a = l
      }
    }
    if (a >= 0) {
...
var p = function () {
  ...
  t.prototype.decodeRow = function (e, n, i) {
    ...
    var u = this.decodeMiddle(n, i, this$decodeRowStringBuffer)
  ...
  t.prototype.decodeMiddle = function (e, n, r) {
    ...
    for (var s = e.getSize(), u = n[1], l = 0, d = 0; d < 5 && u < s; d++) {
      var p = f.decodeDigit(e, this$decodeMiddleCounters, u, f.L_AND_G_PATTERNS)
      r += String.fromCharCode("0".charCodeAt(0) + p % 10)
      try {
        for (i = undefined, m = h(this$decodeMiddleCounters), b = m.next(), undefined; !b.done; b = m.next()) {
          var m
          var b
          u += b.value
        }
  ...
  t.extensionChecksum = function (t) {
    for (var /* [auto-meaningful-name] */t$length = t.length, n = 0, r = t$length - 2; r >= 0; r -= 2) {
      n += t.charAt(r).charCodeAt(0) - "0".charCodeAt(0)
    }
    n *= 3
    for (r = t$length - 1; r >= 0; r -= 2) {
      n += t.charAt(r).charCodeAt(0) - "0".charCodeAt(0)
    }
    return (n *= 3) % 10
  }
...
var b = function () {
  ...
  t.prototype.decodeMiddle = function (t, e, n) {
    ...
    for (var a = t.getSize(), s = e[1], u = 0, l = 0; l < 2 && s < a; l++) {
      var d = f.decodeDigit(t, this$decodeMiddleCounters, s, f.L_AND_G_PATTERNS)
      n += String.fromCharCode("0".charCodeAt(0) + d % 10)
      try {
        for (r = undefined, h = m(this$decodeMiddleCounters), p = h.next(), undefined; !p.done; p = h.next()) {
          var h
          var p
          s += p.value
        }
...
var y = function (t) {
  ...
  e.prototype.decodeRow = function (t, n, u) {
    ...
    if (null != f) {
      var _ = new s.a(p$rowOffset, t)
      f.foundPossibleResultPoint(_)
    }
    var y = e.decodeEnd(n, p$rowOffset)
    if (null != f) {
