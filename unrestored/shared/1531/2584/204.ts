...
function d(e, t) {
  ...
  var u = Math.sqrt(i / r);
  var d = o / (2 * Math.sqrt(i * r));
  var h = d < 1 ? u * Math.sqrt(1 - d * d) : 0;
  var p = d < 1 ? (d * u - c) / h : -c + u;
  function _(e) {
    var n = t ? t * e / 1e3 : e;
    n = d < 1 ? Math.exp(-n * d * u) * (1 * Math.cos(h * n) + p * Math.sin(h * n)) : (1 + p * n) * Math.exp(-n * u);
...
function D(e, t) {
  return l.dom(e) && !l.inp(e) && (!l.nil(S(e, t)) || l.svg(e) && e[t]) ? "attribute" : l.dom(e) && b(o, t) ? "transform" : l.dom(e) && "transform" !== t && B(e, t) ? "css" : null != e[t] ? "object" : void 0;
}
function I(e) {
  if (l.dom(e)) {
...
function ne(e) {
  ...
  function A(e) {
    for (var t = 0, n = f.animations, r = n.length; t < r;) {
      ...
      for (var l = s(e - u.start - u.delay, 0, u.duration) / u.duration, d = isNaN(l) ? 1 : u.easing(l), h = u.to.strings, p = u.round, _ = [], A = u.to.numbers.length, g = void 0, m = 0; m < A; m++) {
        var y = void 0;
        var b = u.to.numbers[m];
        var w = u.from.numbers[m] || 0;
        y = u.isPath ? H(u.value, d * b, u.isPathTargetInsideSVG) : w + d * (b - w);
        if (p) {
          if (!(u.isColor && m > 2)) {
            y = Math.round(y * p) / p;
          }
...
ne.stagger = function (e, t) {
  ...
  var u = "last" === a;
  var f = l.arr(e);
  var d = f ? parseFloat(e[0]) : parseFloat(e);
  var h = f ? parseFloat(e[1]) : 0;
  var p = O(f ? e[1] : e) || 0;
  var _ = t.start || 0 + (f ? d : 0);
  var g = [];
  var v = 0;
  return function (e, t, l) {
    ...
    return _ + (f ? (h - d) / v : d) * (Math.round(100 * g[t]) / 100) + p;
  };
};
