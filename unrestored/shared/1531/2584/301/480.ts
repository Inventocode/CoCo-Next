...
exports.position_dropdown = function (e, t) {
  ...
  if ("first" === t$source_block$workspace$options$dropdown.menu.align) {
    var v = t.image_element ? Number(t.image_element.getAttribute("x")) : t.text_element ? Number(t.text_element.getAttribute("x")) : NaN
    v = isNaN(v) ? t$source_block$workspace$options$dropdown.padding : v
  ...
  } else if ("center" === t$source_block$workspace$options$dropdown.menu.align) {
    if (e.parentElement) {
    ...
    if ("left" === t$source_block$workspace$options$dropdown.menu.align) {
      c[0] = Math.min(c[0], a.width + s[0] - p.width)
    }
  }
  return c
