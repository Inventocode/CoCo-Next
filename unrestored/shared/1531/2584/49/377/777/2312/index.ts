    ...
    if (i !== l.EventType.DISCONNECT) {
      if (i !== l.EventType.DELETE_BLOCK) {
        if (i !== l.EventType.DELETE_COMMENT) {
          ...
          var f = o.target_id;
          var d = this.Blink.mainWorkspace.workspace_comment_db.get(f);
          if (!d) {
            return;
          }
          if (g = this.Blink.json.comment_to_json(d).parent_id) {
            var h = this.Blink.mainWorkspace.get_element_from_db(g);
            if (!h) {
