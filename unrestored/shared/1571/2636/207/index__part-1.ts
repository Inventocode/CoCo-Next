...
var D = new RegExp("return \\/\\* blockId=(\\w+);blockType=".concat($_209_index.PROCEDURE_BLOCK_TYPES.RETURN, " \\*\\/"))
function M(e, t) {
  var n = new RegExp("await (?:Coco.tryExecute\\()?".concat(e.map(function (e) {
    return e.replace(/\$/g, "\\$")
  }).join("|"), " \\/\\* blockId=(\\w+);blockType=").concat($_209_index.PROCEDURE_BLOCK_TYPES.CALL_RETURN, " \\*\\/")).exec(t)
  if (n) {
    return n[1]
  }
}
