...
function s(e, t, n, r, c, u, l) {
  ...
  for (var f = [], d = 0, h = e; d < h.length; d++) {
    ...
    if (Module_252.e(p)) {
      ...
      if (!c || !(p$value in c)) {
        throw new Module_808.d(p$value, l)
      }
      var A = c[p$value]
      if (Module_252.b(p)) {
      ...
      } else if (Module_252.c(p)) {
        var g = "string" === typeof p.style ? r.date[p.style] : Module_252.d(p.style) ? p.style.parsedOptions : undefined
        f.push({
          type: 0,
          value: n.getDateTimeFormat(t, g).format(A)
        })
      } else if (Module_252.l(p)) {
        g = "string" === typeof p.style ? r.time[p.style] : Module_252.d(p.style) ? p.style.parsedOptions : undefined
        f.push({
          type: 0,
          value: n.getDateTimeFormat(t, g).format(A)
        })
      } else if (Module_252.f(p)) {
        g = "string" === typeof p.style ? r.number[p.style] : Module_252.g(p.style) ? p.style.parsedOptions : undefined
        f.push({
          type: 0,
        ...
        if (Module_252.k(p)) {
          ...
          var y = c[p$value1]
          if (!a(y)) {
            throw new Module_808.c(p$value1, "function", l)
          }
        ...
        if (Module_252.j(p)) {
          if (!(w = p.options[A] || p.options.other)) {
            throw new Module_808.b(p.value, A, Object.keys(p.options), l)
          }
          f.push.apply(f, s(w.value, t, n, r, c))
        } else if (Module_252.h(p)) {
          var w
          if (!(w = p.options["=" + A])) {
            if (!Intl.PluralRules) {
              throw new Module_808.a("Intl.PluralRules is not available in this environment.\nTry polyfilling it using \"@formatjs/intl-pluralrules\"\n", "MISSING_INTL_API", l)
            }
